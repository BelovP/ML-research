...
def load(dir_path,load_to_memory=False,dtype=np.float64):
    ...
    meta_file_str = os.path.join(dir_path, 'k_points_rand_meta')
    ...
    train_file,valid_file,test_file = [os.path.join(dir_path, 'k_points_rand_' + ds + '_set.amat') for ds in ['train','valid','test']]
    # Get data
